...
function SuccessScreen({ email, onLogin }: { email: string; onLogin: () => void }) {
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 32 }} className="bg-background">
      <View style={{ width: 80, height: 80, borderRadius: 24, backgroundColor: ACCENT + '20', alignItems: 'center', justifyContent: 'center', marginBottom: 24 }}>
        <MailCheck size={40} color={ACCENT} />
      </View>
      <Text className="mb-3 text-center text-2xl font-bold text-foreground">Перевір пошту</Text>
      <Text className="mb-1 text-center text-base text-muted-foreground">Ми надіслали лист підтвердження на</Text>
      <Text className="mb-8 text-center text-base font-semibold" style={{ color: ACCENT }}>{email}</Text>
...
const TEXT_FIELDS = [
  { name: 'firstName' as const, label: "Імʼя",     icon: User, placeholder: 'Юрко',           autoCapitalize: 'words' as const },
  { name: 'lastName'  as const, label: 'Прізвище',  icon: User, placeholder: 'Іванченко',       autoCapitalize: 'words' as const },
  { name: 'email'     as const, label: 'Email',     icon: Mail, placeholder: 'you@example.com',  autoCapitalize: 'none'  as const },
];
...
export function RegisterWidget() {
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      const formFields = new Set<string>(['firstName', 'lastName', 'email', 'password', 'confirmPassword']);
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={{ flex: 1, justifyContent: 'center', paddingHorizontal: 24, paddingBottom: 40, paddingTop: 80 }}>
          ...
          <View style={{ marginBottom: 40 }}>
            ...
            <Text className="text-base text-muted-foreground">Створи акаунт HoldMeUp, щоб відстежувати свої підйоми</Text>
          </View>
          ...
          <View style={{ gap: 20 }}>
                  ...
                  render={({ field: { onChange, onBlur, value } }) => (
                    <View style={{ position: 'relative' }}>
                      ...
                      <Input
                        ...
                        {...(name === 'email' ? { keyboardType: 'email-address' as const, autoComplete: 'email' as const } : {})}
            ...
            <View style={{ gap: 8 }}>
              ...
              <Controller
                ...
                name="password"
                render={({ field: { onChange, onBlur, value } }) => (
                  <PasswordInput value={value} onChangeText={onChange} onBlur={onBlur} placeholder="••••••••" autoComplete="new-password" />
                )}
              />
            ...
            <View style={{ gap: 8 }}>
              ...
              <Controller
                ...
                render={({ field: { onChange, onBlur, value } }) => (
                  <PasswordInput value={value} onChangeText={onChange} onBlur={onBlur} placeholder="••••••••" />
                )}
              />
          ...
          <View style={{ marginTop: 32, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
            ...
            <TouchableOpacity onPress={() => router.back()}>
              <Text className="text-sm font-semibold" style={{ color: ACCENT }}>Увійти</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ScrollView>
