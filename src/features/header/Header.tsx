...
function AppHeader() {
  ...
  const { isLoading, isError, refetch } = useMyGymMembershipsQuery(user?.id);
                ...
                <DropdownMenuContent sideOffset={8} align="end" className="min-w-[180px]">
                  {isError ? (
                    ...
                    <DropdownMenuItem onPress={() => router.push('/gym/manage' as never)}>
                      ...
                      <Text className="text-sm font-medium" style={{ color: ACCENT }}>Керувати залами</Text>
                    </DropdownMenuItem>
                  ) : (
                    <>
                      <DropdownMenuRadioGroup
                      ...
                      <DropdownMenuItem onPress={() => router.push('/gym/manage' as never)}>
                        <Icon as={Settings2} size={14} color={ACCENT} />
                        <Text className="text-sm" style={{ color: ACCENT }}>Керувати залами</Text>
                      </DropdownMenuItem>
                    </>
                  )}
                </DropdownMenuContent>
